...
import { useEditorSettings, useFormContext } from '@/form-builder/context/EditorFormContext'
...
export function NameField({ field }: FieldProps) {
  ...
  const hasError = (field.state.meta.isTouched && field.state.meta.errors.length > 0) || isDuplicate
  ...
  return (
    <div>
      ...
      />
      {isDuplicate && (
        <p className="field-error mt-1">A field with the name "{effectiveValue}" already exists</p>
      )}
    </div>
  )
}
...
export function LabelField({ field }: FieldProps) {
  ...
  return (
    <TextInput
      ...
      readOnly={locked}
      required
      showError={field.state.meta.isTouched && field.state.meta.errors.length > 0}
  ...
  return (
    <div className={styles.visibilityGrid}>
      ...
      <p className="field-description field-description-errorMessage">
        Hidden fields are not shown to the user but are included in form submissions.
      </p>
    </div>
  )
}
...
export function ConditionsField({ currentFieldId, field }: ConditionsFieldProps) {
  ...
  return (
    <div className={styles.grid}>
      ...
      {!exclude?.includes('hidden') && (
        <form.Field name="hidden">{(f) => <VisibilityField field={f} />}</form.Field>
      )}

      {!exclude?.includes('errorMessage') && (
        <form.Field name="errorMessage">{(f) => <ErrorMessageField field={f} />}</form.Field>
      )}

      {!exclude?.includes('instructions') && (
        <form.Field name="instructions">{(f) => <InstructionsField field={f} />}</form.Field>
      )}

      {!exclude?.includes('placeholder') && (
        <form.Field name="placeholder">{(f) => <PlaceholderField field={f} />}</form.Field>
      )}
      {!exclude?.includes('defaultValue') && (
        <form.Field name="defaultValue">{(f) => <DefaultValueField field={f} />}</form.Field>
      )}
      {children}
    </div>
  )
...
export function GeneralFields({
  ...
  return (
    <div className={styles.grid}>
      ...
      <form.Field name="required">{(f) => <RequiredField field={f} />}</form.Field>
      {children}
    </div>
  )
}
