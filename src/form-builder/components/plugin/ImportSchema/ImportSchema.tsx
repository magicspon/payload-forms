...
import type { FormPage, FormRow } from '../../../utils/formTree'

import { createDefaultField } from '../../../fieldSchema'
import { fieldTypes } from '../../../utils/fieldTypes'
import { appendRowToPage, getAllFields } from '../../../utils/formTree'

// 'message' has no name/label; 'array' requires sub-field authoring — exclude both
const importableFieldTypes = fieldTypes.filter(
	(t) => t.value !== 'message' && t.value !== 'array',
)
...
function isOptionsType(
	t: FieldType | null,
): t is 'checkbox' | 'radio' | 'select' {
...
function computeNameErrors(
	...
	for (let i = 0; i < mappings.length; i++) {
		const m = mappings[i]
		if (m.fieldType === null) {continue} // skipped rows don't participate
		const n = m.name.trim()
		if (!n) {
...
export function ImportSchema() {
				...
				setMappings(
					headers.map<Mapping>((h) => {
						...
						return {
							...
							fieldType,
							header: h,
							options: isOptionsType(fieldType)
								? [{ label: '', value: '' }]
								: [],
						}
					}),
	...
	const existingNames = React.useMemo<Set<string>>(() => {
		if (insertMode === 'replace') {return new Set()}
		return new Set(
			getAllFields(pages)
				.filter((f) => 'name' in f)
				.map((f) => f.name),
	...
	function handleGenerate() {
		...
		if (nameErrors.size > 0) {return} // guard — button is disabled, but be safe
		...
		if (insertMode === 'replace') {
			...
			let updated = pages
			for (const row of newRows) {updated = appendRowToPage(updated, pageId, row)}
			setPages(updated)
		} else {
			const page: FormPage = {
				id: nanoid(),
		...
		<div className="import-schema">
			...
			{hasMappings && (
				<div className="import-schema__insert-mode">
					<p className="import-schema__label">
						Where should the fields be added?
					</p>
