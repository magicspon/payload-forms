...
import { PlusIcon, Popup, PopupList, TextInput, XIcon, DragHandleIcon } from '@payloadcms/ui'
import cx from 'clsx'
...
function SubFieldChip({ index, onDelete, onReorder, onUpdate, subField }: SubFieldChipProps) {
  ...
  React.useEffect(() => {
    ...
    return combine(
      draggable({
        element: rowEl,
        dragHandle: handleEl,
        getInitialData: () => ({ type: 'sub-field-chip', index, subField }),
      ...
      dropTargetForElements({
        element: rowEl,
        canDrop: ({ source }) =>
          source.data.type === 'sub-field-chip' && source.data.index !== index,
        getData: ({ input, element }) =>
          attachClosestEdge(
            { type: 'sub-field-target', index },
  ...
  return (
    ...
    >
      ...
      <span className={styles.handle} ref={handleRef} title="Drag to reorder">
        <DragHandleIcon />
      </span>

      <div className={styles.chipContent}>
        <div className={styles.chipTitle}>{subField.label || '(untitled)'}</div>
        <div className={styles.chipType}>{subField.type}</div>
      </div>

      <SubFieldEditorDrawer onChange={onUpdate} subField={subField} />

      <button className={styles.iconButtonZ} onClick={onDelete} title="Delete field" type="button">
  ...
  return (
    <div className={styles.rowItem}>
      <div className={styles.rowHeader}>
        <span className={styles.rowHeaderLabel}>Row {rowIndex + 1}</span>
        <button
      ...
      <div className={styles.dropZone}>
        {row.columns.length === 0 ? (
          <span className={styles.dropZoneEmpty}>Add a field below</span>
        ) : (
          <Inline className={styles.chipWrap}>
            {row.columns.map((subField, subIndex) => (
              <SubFieldChip
                index={subIndex}
                key={subField.id}
      ...
      <Popup
        button={
          <button className={styles.addFieldButton} data-testid="add-sub-field" type="button">
            <PlusIcon />
...
export function SubFieldsManager({
  label = 'Sub-fields layout',
  onChange,
  rows,
  ...
  function addField(rowId: string, type: ArrayItemFieldType) {
    ...
    onChange(
      rows.map((row) => (row.id === rowId ? { ...row, columns: [...row.columns, newField] } : row)),
    )
  }
  ...
  function removeField(rowId: string, fieldId: string) {
    onChange(
      rows.map((row) =>
        row.id === rowId ? { ...row, columns: row.columns.filter((f) => f.id !== fieldId) } : row,
      ),
    )
  }
  ...
  return (
    <Stack className={styles.subFieldsContainer}>
      ...
      <button className={styles.addRowButton} data-testid="add-row" onClick={addRow} type="button">
        <PlusIcon />
        Add row
      </button>
    </Stack>
