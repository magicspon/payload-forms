...
function SubFieldEditorContent({ drawerSlug, onChange, subField }: { drawerSlug: string } & Props) {
  ...
  return (
    <>
      ...
      <Drawer slug={drawerSlug} title={`Edit: ${title}`}>
        <SubFieldEditorContent drawerSlug={drawerSlug} onChange={onChange} subField={subField} />
      </Drawer>
    </>
  )
}
