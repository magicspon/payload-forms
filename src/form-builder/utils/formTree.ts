...
export function findPage(pages: FormPage[], pageId: string): FormPage | undefined {
...
export function insertRowBefore(
  ...
  return pages.map((page) => ({
    ...page,
    rows: page.rows.flatMap((row) => (row.id === targetRowId ? [newRow, row] : [row])),
  }))
}
...
export function insertRowAfter(
  ...
  return pages.map((page) => ({
    ...page,
    rows: page.rows.flatMap((row) => (row.id === targetRowId ? [row, newRow] : [row])),
  }))
}

export function insertRowAtIndex(pages: FormPage[], index: number, newRow?: FormRow): FormPage[] {
  return pages.map((page) => ({
    ...page,
    rows: [
...
export function appendRowToPage(pages: FormPage[], pageId: string, newRow: FormRow): FormPage[] {
  return pages.map((page) =>
...
export function appendFieldToRow(pages: FormPage[], rowId: string, newField: Field): FormPage[] {
  return pages.map((page) => ({
    ...page,
    rows: page.rows.map((row) =>
...
export function prependFieldToRow(pages: FormPage[], rowId: string, newField: Field): FormPage[] {
  return pages.map((page) => ({
    ...page,
    rows: page.rows.map((row) =>
...
export function replaceField(pages: FormPage[], fieldId: string, newField: Field): FormPage[] {
  return pages.map((page) => ({
    ...page,
    rows: page.rows.map((row) => ({
      ...row,
      columns: row.columns.map((field) => (field.id === fieldId ? newField : field)),
    })),
  }))
}
...
export function updateRow(
  ...
  return pages.map((page) => ({
    ...page,
    rows: page.rows.map((row) => (row.id === rowId ? { ...row, ...updates } : row)),
  }))
}
...
export function moveRow(
  ...
  result = result.map((page) => {
    ...
    return {
      ...page,
      rows: position === 'start' ? [found.row, ...page.rows] : [...page.rows, found.row],
    }
  })
