import type { TextField } from '@/shared/fieldSchema'
...
describe('insertFieldBefore', () => {
  it('inserts field before the target', () => {
    ...
    expect(result[0].rows[0].columns.map((f) => f.id)).toEqual(['f1', 'new', 'f2'])
  })
})
...
describe('replaceField', () => {
  it('replaces the field entirely', () => {
    ...
    expect((result[0].rows[0].columns[0] as TextField).label).toBe('Replaced')
  })
})
...
describe('moveField', () => {
  it('moves field to end of another row', () => {
    const pages = [makePage('p1', makeRow('r1', 'f1', 'f2'), makeRow('r2', 'f3'))]
...
describe('reorderFields', () => {
  it('moves field from index to another index', () => {
    ...
    expect(result[0].rows[0].columns.map((f) => f.id)).toEqual(['f2', 'f3', 'f1'])
  })
