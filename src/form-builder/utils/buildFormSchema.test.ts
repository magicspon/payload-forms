import type { ArrayItemField, Field } from '@/shared/fieldSchema'
...
function field(id: string, overrides: { type: Field['type'] } & Partial<Field>): Field {
...
describe('buildFormSchema', () => {
  it('returns a JSON schema object', () => {
    const schema = buildFormSchema({
      fields: [field('f1', { name: 'first_name', type: 'text', required: true })],
    })
  ...
  it('array field produces array type with item properties from a single row', () => {
    ...
    expect(schema.properties?.['contacts']?.items?.properties).toHaveProperty('firstName')
    expect(schema.properties?.['contacts']?.items?.properties).toHaveProperty('emailAddress')
  })
  ...
  it('file field produces array type', () => {
    const schema = buildFormSchema({
      fields: [field('f1', { name: 'attachments', type: 'file', required: true })],
    }) as { properties?: Record<string, { type?: string }> }
