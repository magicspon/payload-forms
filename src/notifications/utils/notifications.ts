import type { Condition, ConditionOperator, FieldConditions } from '@/shared/fieldSchema'
...
function evaluateCondition(
  ...
  if (condition.operator === 'hasChanged') {
    const prev = String(
      (previousData?.[condition.field] as bigint | boolean | number | string | undefined) ?? '',
    )
    const curr = String(
      (submissionData[condition.field] as bigint | boolean | number | string | undefined) ?? '',
    )
    return prev !== curr
  }
