...
function Chip({ onRemove, value }: ChipProps) {
  ...
  return (
    <span className={styles.chip} data-token={token || undefined}>
      {value}
      <button
  ...
  return (
    <div className="field-type text" style={{ flex: '1 1 auto' }}>
      ...
      <div className={styles.inputWrapper}>
        {tokens.map((token, i) => (
          <Chip key={`${token}-${i}`} onRemove={() => removeToken(i)} value={token} />
        ))}
      </div>
      <div className={styles.inputWrapper}>
        <input
          ...
          placeholder={tokens.length === 0 ? 'Enter email or insert token…' : ''}
          type="email"
          value={inputValue}
        />
        ...
        <Popup
          button={
            <button aria-label="Insert token" className={styles.insertButton} type="button">
              <span>{'{ }'}</span>
              <span>Insert</span>
            </button>
          }
        ...
        >
          <PopupList.ButtonGroup>
            {formFields.map((field) => (
              <PopupList.Button key={field.name} onClick={() => addToken(`{{${field.name}}}`)}>
      ...
      {inputError && <p className={`field-error ${styles.errorText}`}>{inputError}</p>}
    </div>
  )
}
