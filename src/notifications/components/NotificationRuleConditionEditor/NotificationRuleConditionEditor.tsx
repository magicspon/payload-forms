...
function getOperatorsForFieldType(fieldType: Field['type'] | undefined) {
	...
	if (fieldType === 'number') {return conditionOperators}
...
export function NotificationRuleConditionEditor({
	path,
}: NotificationRuleConditionEditorProps) {
	...
	const formId =
		formValue && typeof formValue === 'object' ? formValue.id : formValue
	...
	useEffect(() => {
		...
		fetch(`${serverURL}/api/forms/${formId}?depth=0`, {
			...
			.then((data: { pages?: FormPage[] } | null) => {
				...
				setAvailableFields(
					getAllFields(pages).filter((f) => f.type !== 'message'),
				)
			})
		...
		return (
			<div className={styles.emptyState}>
				<p className={styles.emptyText}>
					Select a form above to configure field conditions
				</p>
			</div>
		)
	}
	...
	if (availableFields.length === 0) {
		return (
			<div className={styles.emptyState}>
				<p className={styles.emptyText}>
					No fields found on the selected form
				</p>
			</div>
		)
	}
		...
		<div className={styles.container}>
			...
			{conditionsList.length > 0 ? (
				...
				<p className={styles.noConditionsText}>
					No conditions set. This notification fires whenever the trigger event
					occurs.
				</p>
			)}
