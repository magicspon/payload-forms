...
export async function SubmissionsView(props: Props) {
	...
	const [form, submissionsResult] = await Promise.all([
		...
		}) as Promise<{ docs: SubmissionDoc[]; hasNextPage: boolean; hasPrevPage: boolean; page: number; totalPages: number }>,
	])

	const pages = form.pages ?? []
	const nonMessageFields = getAllFields(pages).filter(
		(f): f is Exclude<typeof f, { type: 'message' }> =>
			f.type !== 'message' && !f.hidden,
	)
