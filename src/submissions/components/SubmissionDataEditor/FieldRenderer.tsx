...
export function FieldRenderer({ field, formUploadsSlug, onChange, value }: FieldRendererProps) {
  switch (field.type) {
    case 'array':
      return (
        <ArrayFieldRenderer
    ...
    case 'date':
      return (
        <DateFieldRenderer field={field} onChange={onChange} value={value as string | undefined} />
      )
    case 'email':
    case 'text':
      return (
        <TextFieldRenderer field={field} onChange={onChange} value={value as string | undefined} />
      )
    case 'file':
      return (
        <FileFieldRenderer
    ...
    case 'radio':
      return (
        <RadioFieldRenderer field={field} onChange={onChange} value={value as string | undefined} />
      )
    case 'select':
      return (
        <SelectFieldRenderer
...
function DateFieldRenderer({
  ...
  return (
    <div className="field-type date">
      ...
      <DatePicker
        onChange={(val: Date | undefined) => onChange(val ? val.toISOString() : null)}
        placeholder={field.placeholder}
        value={dateValue}
      />
    </div>
...
function ArrayFieldRenderer({
  ...
  const updateRow = useCallback(
    (index: number, fieldName: string, fieldValue: unknown) => {
      const next = rows.map((row, i) => (i === index ? { ...row, [fieldName]: fieldValue } : row))
      onChange(next)
    },
    [rows, onChange],
  )
