...
/** Find a field by name anywhere in the collection tree. */
function findField(
	...
	function search(
		...
		for (const f of fields) {
			if ('name' in f && f.name === name) {return f as Record<string, unknown>}
			if ('fields' in f && Array.isArray(f.fields)) {
	...
	return search(collection.fields)
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Multitenancy — team field
// ─────────────────────────────────────────────────────────────────────────────

describe('formsPlugin — multitenancy', () => {
	it('adds a team field to forms, submissions and form-uploads when enabled', () => {
		const config = formsPlugin({
			multitenancy: { enabled: true },
		})(baseConfig())

		for (const slug of ['forms', 'submissions', 'form-uploads']) {
			const col = findCollection(config, slug)
			expect(fieldNames(col)).toContain('team')
		}
	})

	it('does not add a team field when multitenancy is disabled', () => {
		const config = formsPlugin({
		...
		for (const slug of ['forms', 'submissions', 'form-uploads']) {
			const col = findCollection(config, slug)
			expect(fieldNames(col)).not.toContain('team')
		}
	})

	it('does not add a team field when multitenancy is omitted', () => {
		const config = formsPlugin()(baseConfig())
	...
	it('team field relationTo defaults to "teams"', () => {
		const config = formsPlugin({ multitenancy: { enabled: true } })(
			baseConfig(),
		)
		const col = findCollection(config, 'forms')
		const teamField = findField(col, 'team') as
			| { relationTo: string }
			| undefined
		expect(teamField?.relationTo).toBe('teams')
	})

	it('team field uses custom teamsCollection when provided', () => {
		const config = formsPlugin({
			multitenancy: { enabled: true, teamsCollection: 'organisations' },
		})(baseConfig())
		const col = findCollection(config, 'forms')
		const teamField = findField(col, 'team') as
			| { relationTo: string }
			| undefined
		expect(teamField?.relationTo).toBe('organisations')
	})

	it('adds a unique (team, slug) index to forms when multitenancy is enabled', () => {
		const config = formsPlugin({ multitenancy: { enabled: true } })(
			baseConfig(),
		)
		const col = findCollection(config, 'forms')
		const indexes = col.indexes as
			| { fields: string[]; unique?: boolean }[]
			| undefined
		expect(indexes).toEqual(
			expect.arrayContaining([
				expect.objectContaining({ fields: ['team', 'slug'], unique: true }),
			]),
		)
	})

	it('does not add the unique (team, slug) index when multitenancy is disabled', () => {
		const config = formsPlugin({ multitenancy: { enabled: false } })(
			baseConfig(),
		)
		const col = findCollection(config, 'forms')
		const indexes = col.indexes as { fields: string[] }[] | undefined
		const hasTeamSlug = indexes?.some(
			(i) => i.fields.includes('team') && i.fields.includes('slug'),
		)
		expect(hasTeamSlug).toBeFalsy()
	})

	it('applies fieldOverride to the team field', () => {
		const config = formsPlugin({
			multitenancy: {
				enabled: true,
				fieldOverride: { required: true },
			},
		})(baseConfig())
		const col = findCollection(config, 'forms')
		const teamField = findField(col, 'team') as
