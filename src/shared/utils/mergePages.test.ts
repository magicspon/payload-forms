...
function textField(id: string, label: string, extra: Partial<Field> = {}): Field {
  return {
    id,
    name: id,
    type: 'text',
...
function makePage(id: string, fields: Field[], overrides: Partial<FormPage> = {}): FormPage {
  return {
    id,
    backButton: 'Back',
    nextButton: 'Next',
...
describe('mergePages — page-level strings', () => {
  ...
  it('uses locale backButton and nextButton when IDs match', () => {
    const canonical = [makePage('p1', [], { backButton: 'Back', nextButton: 'Next' })]
    const locale = [makePage('p1', [], { backButton: 'Tillbaka', nextButton: 'Nästa' })]
    const result = mergePages(canonical, locale)
  ...
  it('canonical page order is always preserved', () => {
    const canonical = [makePage('p1', []), makePage('p2', []), makePage('p3', [])]
    const locale = [makePage('p3', [], { title: 'C' }), makePage('p1', [], { title: 'A' })]
    const result = mergePages(canonical, locale)
...
describe('mergePages — field translatable strings', () => {
  ...
  it('uses locale field placeholder when field IDs match', () => {
    const canonical = [makePage('p1', [textField('f1', 'Name', { placeholder: 'Enter name' })])]
    const locale = [makePage('p1', [textField('f1', 'Namn', { placeholder: 'Ange namn' })])]
    const result = mergePages(canonical, locale)
    expect((col(result, 0) as { placeholder?: string }).placeholder).toBe('Ange namn')
  })

  it('falls back to canonical label when locale field is absent', () => {
    const canonical = [makePage('p1', [textField('f1', 'Name'), textField('f2', 'Email')])]
    const locale = [makePage('p1', [textField('f1', 'Namn')])]
    const result = mergePages(canonical, locale)
  ...
  it('canonical field structure always wins over locale structure', () => {
    const canonical = [makePage('p1', [textField('f1', 'Name', { required: true })])]
    // locale has required: false — should be ignored
    const locale = [makePage('p1', [textField('f1', 'Namn', { required: false })])]
    const result = mergePages(canonical, locale)
...
describe('mergePages — option field labels', () => {
  function radioField(id: string, options: Array<{ label: string; value: string }>): Field {
    return {
      id,
      name: id,
      type: 'radio',
  ...
  it('falls back to canonical option label when locale option is absent', () => {
    ...
    // locale only has 'yes'
    const locale = [makePage('p1', [radioField('f1', [{ label: 'Ja', value: 'yes' }])])]
    const result = mergePages(canonical, locale)
    const opts = (col(result, 0) as WithOptions).options
...
describe('mergePages — message fields', () => {
  it('always uses canonical for message fields regardless of locale', () => {
    ...
    const canonical = [
      makePage('p1', [{ id: 'msg1', type: 'message', hidden: true, richText } as Field]),
    ]
    const locale = [
      makePage('p1', [
        {
