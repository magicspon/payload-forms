...
function SubscribeButton({ children, ...props }: React.ComponentProps<'button'>) {
...
export function useForm({
  ...
  const visiblePageFields =
    page?.rows
      ...
      .filter((f): f is NamedFieldProps => f.type !== 'message' && isVisible(f, values)) ?? []

  async function handleNext() {
    await Promise.all(visiblePageFields.map((f) => form.validateField(f.name, 'blur')))
