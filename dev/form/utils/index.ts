...
import type { ArrayItemValue, FormFieldValue, NamedFieldProps } from '@spon/payload-forms/form'
 ...
 */
export function buildValidatorCache(formSchema: FormData['formSchema']): Map<string, z.ZodTypeAny> {
  ...
  for (const [name, subSchema] of Object.entries(properties)) {
    try {
      cache.set(name, z.fromJSONSchema(subSchema as Parameters<typeof z.fromJSONSchema>[0]))
    } catch {
...
export function makeValidator(field: NamedFieldProps, zodSchema: z.ZodTypeAny | undefined) {
